...
self.onmessage = function (event) {
    const { phase, sharedBuffer, group } = event.data || {}

    if (phase !== 'subtree') {
        self.postMessage({ type: 'error', error: `Unknown phase: ${phase}` })
        return
    }
    ...
    try {
        ...
        const trees = group.prefixes.map(prefixInfo => buildSubTreeForPrefix(text, prefixInfo))
        ...
        self.postMessage({
            type: 'error',
            error: error?.message || 'Unknown error while building subtrees'
        })
    }
}

function buildSubTreeForPrefix(text, prefixInfo) {
