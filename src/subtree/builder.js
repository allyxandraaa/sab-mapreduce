...
export async function buildSubTrees({ sharedBuffer, sPrefixes, config, executeRound }) {
    ...
    const memoryLimit = config?.memoryLimit || 5000
    const numWorkers = config?.numWorkers || 4

    logger.info('SubTree', 'Старт побудови піддерев', {
    ...
    logger.info('SubTree', 'Результат розподілу груп', {
    ...
    if (groups.length) {
        logger.info('SubTree', 'Деталі груп', groups.map((group, index) => ({
            order: index + 1,
            groupId: group.id,
    ...
    if (rounds.length) {
        logger.info('SubTree', 'Розклад раундів', rounds.map((round, roundIndex) => ({
            round: roundIndex + 1,
    ...
    for (let index = 0; index < rounds.length; index++) {
        ...
        logger.info('SubTree', `Виконуємо раунд ${index + 1}/${rounds.length}`, {
        ...
        logger.info('SubTree', `Завершено раунд ${index + 1}/${rounds.length}`, {
        ...
        roundResults.forEach(result => {
            ...
            logger.info('SubTree', 'Отримано результат групи', {
    ...
    logger.info('SubTree', 'Побудову піддерев завершено', {
